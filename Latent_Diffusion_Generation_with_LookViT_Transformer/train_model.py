...
from dataloader import LatentDataset
import torch
...
from utils import cosine_alphas_bar, count_parameters, cold_diffuse
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, train_epoch):
        ...
        for num_iter, (latents) in enumerate(progress_bar):
        ...
        latent_noise = 0.8 * torch.randn(1, 4, 12,12, device=device)
        ...
        with torch.no_grad():
            ...
            fake_sample = vae.decode(fake_latents / 0.13025).sample
        decoded_output = ((fake_sample.squeeze(0).permute(1, 2, 0).cpu().float() * 0.5 + 0.5).clip(0, 1) * 255).to(torch.uint8)
        decoded_image = Image.fromarray(decoded_output.numpy())
        decoded_image.save(f'result_from_epoch{epoch+1}.jpg')